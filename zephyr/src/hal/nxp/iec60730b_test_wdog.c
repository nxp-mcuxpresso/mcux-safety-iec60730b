			...
			const struct device *counter, uint32_t tolerance_percent)
{
	 ...
	 * 1. On first run: Configures and starts the watchdog timer, then waits for a watchdog
	 * reset while counting reference counter ticks.
	 * 2. After watchdog reset: Verifies that the measured counter ticks during the watchdog
	 * timeout period are within the specified tolerance range.
	 */
	ret = hwinfo_get_reset_cause(&cause);
	(void)hwinfo_clear_reset_cause();
	if (ret == 0) {
		/* Check if the reset was caused by a watchdog timeout.
		 * If true, verify that the reference counter ticks are within the acceptable
		 * tolerance range.
		 */
		if (cause & RESET_WATCHDOG) {
			...
			/* Set up dummy reset detection register and mask for FS_WDOG_Check()
			 * validation. The actual reset cause has already been verified/cleared by
			 * hwinfo_get/clear_reset_cause().
			 */
			...
			/* Calculate the expected number of reference counter ticks during one timer
			 * period */
			clock_test_expected =
				((uint64_t)counter_frequency * wdog_timeout_ms) / 1000;
			/* Calculate the tolerance value as a percentage of the expected counter
			 * value */
			clock_test_tolerance =
				((uint64_t)clock_test_expected * tolerance_percent) / 100;
			/* Calculate the upper and lower limits for the counter ticks based on the
			...
			LOG_DBG("Counter ticks=%u [%u %u]\n", WatchdogBackup.counter,
				counter_ticks_limit_low, counter_ticks_limit_high);
			if (FS_WDOG_Check(counter_ticks_limit_high, counter_ticks_limit_low, 2,
					  FALSE, &WatchdogBackup, FALSE, FALSE) != FS_PASS) {
				return IEC60730B_TEST_WDT_ERROR;
			}
			/* Reset was not caused by watchdog. Proceed to configure and start the
			 * watchdog test. */
		} else {
			const char *counter_name = counter->name;
#ifdef CONFIG_COUNTER_MCUX_CTIMER
			if (strstr(counter_name, "ctimer") != NULL) {
				WatchdogBackup.RefTimerBase =
					(uint32_t)((struct mcux_lpc_ctimer_config *)counter->config)
						->base;
#ifdef CONFIG_WDT_MCUX_WWDT
				WatchdogBackup.WdogBase =
					(uint32_t)((const struct mcux_wwdt_config *)wdog->config)
						->base;

				struct wdt_timeout_cfg wdog_config = {
					.flags = WDT_FLAG_RESET_SOC,   /* Reset SoC when watchdog
									  timer expires */
					.window.max = wdog_timeout_ms, /* Expire watchdog after max
									  window */
				};
