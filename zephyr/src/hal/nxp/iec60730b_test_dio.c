...
#define DIO_WAIT_CYCLE 200
	...
	tested_pin.pPort_byte =
		(uint8_t *)&gpio_base->B[port_no][pin]; /* Adress of byte register in GPIO */
	tested_pin.pPort_dir =
		(uint32_t *)&gpio_base->DIR[port_no]; /* Adress of dir register in GPIO */
#ifdef IOCON /* LPC SOCs */
	{
		tested_pin.pPort_Iocon = (uint32_t *)&(
			config->pinmux_base
				->PIO[port_no][pin]); /* Adress of concrete IOCON register */
