...
#define ADC_VOLTAGE_TO_RAW(voltage, vref, resolution)                                              \
	(uint32_t)(((voltage) * (ADC_MAX(resolution))) / (vref))
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
/*!
 * @brief Set tolerance range limits (raw adc value) for adc channel according to expected voltage
 * and ±deviation percentage.
 *
 * @kconfig_dep{CONFIG_IEC60730B_TEST_AIO}
 *
 * This function sets upper and lower limits for a specific adc channel according to the
 * channel_expected_voltage_mv parameter, and allowed deviation percentage defined for the function.
 * The calculated limit values are saved to the limits.
 *
 * @param channel Pointer to the ADC channel specification with parameters from board overlay or
 * entered by user application.
 * @param channel_expected_voltage_mv Voltage signal value in mV that is connected to the measuring
 * ADC channel, and expected to be read by ADC conversion.
 * @param channel_limits Pointer to the structure for allowed limits for the specific channel.
 *
 * @return 0 on success, negative on failure
 */
static int iec60730b_aio_set_channel_limits(struct iec60730b_adc_channel *channel,
					    ...
					    uint32_t channel_expected_voltage_mv)
{
	...
	uint32_t adc_channnel_expected_raw =
		ADC_VOLTAGE_TO_RAW(channel_expected_voltage_mv, vref_mv, resolution);

	/* Check if expected raw value is bigger than allowed deviation % of ADC range to avoid
	 * underflow */
	if (adc_channnel_expected_raw > ADC_RANGE_PERCENT(resolution, allowed_deviation)) {
		...
		channel_limits->low = adc_channnel_expected_raw -
				      ADC_RANGE_PERCENT(resolution, allowed_deviation);
	} else {
	...
	if (adc_channnel_expected_raw < ADC_RANGE_PERCENT(resolution, (100 - allowed_deviation))) {
		...
		channel_limits->high = adc_channnel_expected_raw +
				       ADC_RANGE_PERCENT(resolution, allowed_deviation);
	} else {
...
int iec60730b_aio_init_channel(const struct device *dev, struct iec60730b_adc_channel *channel,
			       uint32_t channel_expected_voltage_mv)
{
	int err;
