		       ...
		       iec60730b_test_ram_type_t type)
{
	...
	res = FS_CM0_RAM_AfterReset((uint32_t)ram, (uint32_t)(ram + ram_size),
				    (uint32_t)backup_size, (uint32_t)backup, march_type);
	...
	res = FS_CM4_CM7_RAM_AfterReset((uint32_t)ram, (uint32_t)(ram + ram_size),
					(uint32_t)backup_size, (uint32_t)backup, march_type);
	...
	res = FS_CM33_RAM_AfterReset((uint32_t)ram, (uint32_t)(ram + ram_size),
				     (uint32_t)backup_size, (uint32_t)backup, march_type);
#else
	return IEC60730B_TEST_NOT_SUPPORTED;
#endif
